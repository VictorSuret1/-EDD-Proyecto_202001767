...
void ArbolAVL::aceptarSolicitud(const QString& correoEmisor, const QString& correoReceptor, Matrix& matrizAmistades) {
    NodoAVL* receptor = buscar(correoReceptor);
    ...
    if (it != receptor->usuario.solicitudesRecibidas.end()) {
        ...
        // Convertir QString a std::string para usar con la matriz
        std::string emisorStd = correoEmisor.toStdString();
        std::string receptorStd = correoReceptor.toStdString();

        matrizAmistades.insertarAmistad(emisorStd, receptorStd);

        // Remover la solicitud de las listas de enviadas y recibidas
        eliminarSolicitud(correoEmisor, correoReceptor);

        std::cout << "Solicitud de amistad aceptada: " << emisorStd << " -> " << receptorStd << "\n";
    } else {
