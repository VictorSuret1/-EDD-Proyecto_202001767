...
void vUser::on_pushButton_3_clicked()
{
    ...
    if (nuevoCorreo != usuarioActual.correo) {
        // Si el correo ha cambiado, necesitamos mover el nodo en el árbol AVL
        // Buscar el nodo actual y eliminarlo
        ...
        usuarioActual.contrasena = nuevaContra;
        usuarioActual.fechaNacimiento = nuevaFechaNacimiento;

        arbol.insertar(usuarioActual);
    } else {
        // Si el correo no ha cambiado, simplemente actualiza los datos en el nodo existente
        ...
        if (nodoUsuario != nullptr) {
            ...
            nodoUsuario->usuario.contrasena = nuevaContra;
...
void vUser::llenarTablaSolicitudes() {
    ...
    for (Solicitud* solicitud : nodoUsuario->usuario.solicitudesRecibidas) {
        ...
        connect(btnAceptar, &QPushButton::clicked, [this, solicitud, nodoUsuario]() {
            ...
            arbol.aceptarSolicitud(solicitud->correoEmisor, solicitud->correoReceptor, matrizAmistades);
...
void mostrarFechasDesdeArbol(NodoABB* nodo) {
    ...
    // Mostrar la fecha del nodo actual
    qDebug() << "Fecha: " << nodo->fecha.toString("dd/MM/yyyy");
...
void vUser::mostrarPublicacionesUsuario(QScrollArea* scrollArea, Usuario* usuarioLogeado, ArbolABB& arbolPublicaciones) {
    ...
    for (Publicacion* publicacion : listaPublicaciones) {
        if (publicacion->correoUsuario == usuarioLogeado->correo ||
            matrizAmistades.sonAmigos(usuarioLogeado->correo.toStdString(), publicacion->correoUsuario.toStdString())) {
...
void vUser::on_pushButton_5_clicked()
{
    ...
    if (dialogo.exec() == QDialog::Accepted) {
        ...
        if (!texto.isEmpty()) {
            ...
            arbolabb.insertarPublicacion(usuarioActual.correo, texto, rutaImagen, QDate::currentDate());
            lista.agregarPublicacion(usuarioActual.correo, texto, rutaImagen, QDateTime::currentDateTime());
            // Actualizar el feed
            mostrarPublicacionesUsuario(ui->scrollArea, &usuarioActual, arbolabb);
            poblarComboBoxFechas();
            poblarComboBoxABB();
...
void vUser::agregarNuevaPublicacion() {
    ...
    if (dialogo.exec() == QDialog::Accepted) {
        ...
        if (!texto.isEmpty()) {
            ...
            // Insertar la publicación en el árbol ABB y la lista doble
...
void vUser::poblarComboBoxFechas() {
    ui->comboBox->clear();  // Limpiar combobox antes de agregar nuevas fechas
    QList<QDate> fechas;
    obtenerFechasDesdeArbol(arbolabb.obtenerRaiz(), fechas);

    // Agregar fechas al combobox
...
void vUser::poblarComboBoxABB() {
    ui->comboBox_3->clear();  // Limpiar combobox antes de agregar nuevas fechas
    QList<QDate> fechas;
    obtenerFechasDesdeArbol(arbolabb.obtenerRaiz(), fechas);
...
void vUser::obtenerFechasDesdeArbol(NodoABB* nodo, QList<QDate>& fechas) {
    if (!nodo) return;

    obtenerFechasDesdeArbol(nodo->izquierda, fechas);
    fechas.append(nodo->fecha);
    obtenerFechasDesdeArbol(nodo->derecha, fechas);
}
