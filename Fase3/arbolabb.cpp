...
NodoABB* ArbolABB::eliminarNodo(NodoABB* nodo, const QDate& fecha) {
    if (nodo == nullptr) {
        return nodo;  // El nodo no se encontró
    }
    ...
    if (fecha < nodo->fecha) {
        ...
        if (nodo->izquierda == nullptr) {
            ...
            limpiarListaPublicaciones(nodo->listaPublicaciones);  // Limpiar lista de publicaciones del nodo
            delete nodo;
            return temp;
        ...
        NodoABB* sucesor = obtenerMinimo(nodo->derecha);
        nodo->fecha = sucesor->fecha;  // Copiar la fecha del sucesor
        nodo->listaPublicaciones = sucesor->listaPublicaciones;  // Copiar publicaciones del sucesor
        nodo->derecha = eliminarNodo(nodo->derecha, sucesor->fecha);  // Eliminar el sucesor
    }
    return nodo;
}

NodoABB* ArbolABB::obtenerMinimo(NodoABB* nodo) {
    NodoABB* actual = nodo;
...
void ArbolABB::limpiarListaPublicaciones(Publicacion* lista) {
    while (lista != nullptr) {
        Publicacion* temp = lista;
