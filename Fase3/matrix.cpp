...
bool Matrix::sonAmigos(const std::string& correo1, const std::string& correo2) {
    ...
    // Encontrar el nodo de la fila correspondiente al usuario 'correo2'
    ListNode* rowNode = findRow(row);

    if (rowNode == nullptr) {
        return false; // La fila no existe, por lo tanto, no son amigos
    }

    // Recorrer los nodos de la matriz en esa fila
    MatrixNode* current = rowNode->access;

    while (current != nullptr) {
        if (current->col == col) {
            // Se encontró una conexión entre los dos usuarios
            return true;
        }
        current = current->right; // Moverse al siguiente nodo a la derecha
    }

    return false; // No se encontró una conexión, no n amigos
}
...
ListNode* Matrix::findRow(int row) {
    ListNode* current = rowHeader->head;
    while (current != nullptr) {
